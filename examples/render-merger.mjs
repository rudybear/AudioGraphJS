...
async function main() {
  ...
  const spec = {
    sampleRate: sr,
    nodes: [
      { id: 'oscL', kind: 'oscillator', params: { frequency: 440, startTime: 0 } },
      { id: 'oscR', kind: 'oscillator', params: { frequency: 660, startTime: 0 } },
      { id: 'merge', kind: 'channel-merger', params: { numberOfInputs: 2 } },
    ],
    connections: [
      { from: { node: 'oscL' }, to: { node: 'merge' } },
    ],
  };
  const g = buildGraph(ctx, spec);
  // Route oscL to input 0, oscR to input 1
  g.nodes.get('oscL')?.connect(g.nodes.get('merge'), 0, 0);
  g.nodes.get('oscR')?.connect(g.nodes.get('merge'), 0, 1);
  g.nodes.get('merge')?.connect(ctx.destination);
  ...
  const outPath = path.join(__dirname, 'output-merger.wav');
