...
async function main() {
  ...
  const spec = {
    sampleRate: sr,
    nodes: [
      { id: 'osc', kind: 'oscillator', params: { type: 'square', frequency: 220, pulseWidth: 0.25, startTime: 0 } },
    ],
    connections: [],
  };
  const g = buildGraph(ctx, spec);
  g.nodes.get('osc')?.connect(ctx.destination);
  const rendered = await ctx.startRendering();
  const out = rendered.getChannelData(0);
  const wav = writeWavPCM16LE({ samples: out, sampleRate: sr, numChannels: 1 });
  const outPath = path.join(__dirname, 'output-oscillator-pwm.wav');
