...
async function main() {
  ...
  const impulse = ctx.createBuffer(1, 4, sr);
  impulse.getChannelData(0)[0] = 1.0;
  const spec = {
    sampleRate: sr,
    nodes: [
      { id: 'src', kind: 'audio-buffer-source', params: { buffer: impulse, startTime: 0 } },
      { id: 'dly', kind: 'delay', params: { maxDelayTime: 1, delayTime: 0.2 } },
    ],
    connections: [ { from: { node: 'src' }, to: { node: 'dly' } } ],
  };
  ...
  const outPath = path.join(__dirname, 'output-delay.wav');
