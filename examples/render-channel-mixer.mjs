...
async function main() {
  ...
  const spec = {
    sampleRate: sr,
    nodes: [
      { id: 'osc', kind: 'oscillator', params: { frequency: 440, startTime: 0 } },
      { id: 'mix', kind: 'channel-mixer', params: { outputChannels: 2, channelInterpretation: 'speakers' } },
    ],
    connections: [ { from: { node: 'osc' }, to: { node: 'mix' } } ],
  };
  ...
  const outPath = path.join(__dirname, 'output-channel-mixer.wav');
