...
async function main() {
  ...
  const supported = (probe && (('positionX' in probe) || typeof probe.setPosition === 'function'));
  if (!supported) {
    console.log('PannerNode not supported in this Node engine. Skipping render.');
    return;
  }
  const spec = {
    sampleRate: sr,
    nodes: [
      { id: 'osc', kind: 'oscillator', params: { frequency: 330, startTime: 0 } },
      { id: 'pan3d', kind: 'panner', params: { panningModel: 'HRTF', distanceModel: 'inverse' } },
    ],
    connections: [ { from: { node: 'osc' }, to: { node: 'pan3d' } } ],
  };
  const g = buildGraph(ctx, spec);
  g.nodes.get('pan3d')?.connect(ctx.destination);
  ...
  const outPath = path.join(__dirname, 'output-panner.wav');
