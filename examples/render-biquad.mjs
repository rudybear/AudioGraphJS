...
async function main() {
  ...
  const spec = {
    sampleRate: sr,
    nodes: [
      { id: 'osc', kind: 'oscillator', params: { frequency: 880, startTime: 0 } },
      { id: 'filt', kind: 'biquad-filter', params: { type: 'lowpass', frequency: 1000, Q: 0.707 } },
    ],
    connections: [ { from: { node: 'osc' }, to: { node: 'filt' } } ],
  };
  const g = buildGraph(ctx, spec);
  g.nodes.get('filt')?.connect(ctx.destination);
  const rendered = await ctx.startRendering();
  const out = rendered.getChannelData(0);
  const wav = writeWavPCM16LE({ samples: out, sampleRate: sr, numChannels: 1 });
  const outPath = path.join(__dirname, 'output-biquad.wav');
